...
- **PM2** — الإشراف على العملية مع `treekill:false` وdrain بلا سقف (ADR-021/022) — ملف العقدة `ecosystem.<node>.config.cjs` (المتعقَّب `ecosystem.config.example.cjs` قالب مرجعي فقط، B-115).
...
**Process management:** PM2 supervises nassaj-dev with `treekill:false`, allowing
orphaned vendor workers (Workflows) to outlive a restart, coupled with an
unbounded drain (`DRAIN_TIMEOUT_MS=0`, `kill_timeout=24h`) to let live roles
finish before the parent is killed.

**Per-node ecosystem files (B-115):** each node runs from its **own**
`ecosystem.<node>.config.cjs` (e.g. `ecosystem.nassaj.config.cjs`), generated by
`bootstrap-node.sh` with all host values inline (`cwd`, `SERVER_PORT`,
`DATABASE_PATH`, `JWT_SECRET`, WebAuthn origins). These node files — and any local
`ecosystem.config.cjs` — are **git-ignored** so host secrets never enter the repo.
The only tracked file is `ecosystem.config.example.cjs`, a **host-neutral
reference template that must NOT be run directly**: with no `.env`/inline values,
`SERVER_PORT` is undefined and the app falls back to port 3001 → EADDRINUSE →
crash-loop (the traventure outage, 2026-06-30). It exists only to keep the
unified B-N-DRAIN structural keys (`treekill`/`kill_timeout`/drain/bind-window)
reproducible across the fleet when a node file is generated.
...
it does **not** assume a runnable `ecosystem.config.cjs`. It guards with
`pm2 describe` (exit code 4 if process missing) and warns of
`treekill`/`kill_timeout` divergence. Its printed recovery commands reference the
node file (`$ECOSYSTEM`, default placeholder `ecosystem.<node>.config.cjs`).
