...
- **محرّك:** سكربت bash في `/home/nassaj/Project/nassaj-ops/scripts/runner/bin/nassaj-runner.sh`
- **التشغيل:** عبر cron النظام كل ١٥ دقيقة (*)
...
| الطبقة | المسار | الدور |
|--------|--------|--------|
| **محرّك bash** | `nassaj-ops/scripts/runner/` | آلة حالات، أمان، flock، حصص |
...
كل دورة cron (كل ١٥ دقيقة)، المنوال ينفّذ **خطوة واحدة** فقط من رحلة طويلة متعددة الأطوار.
...
**الـ verdict دائماً:** opus (أو fable)  
**الـ gate دائماً:** fable  
...
أضِف السطر:
```bash
*/15 * * * * /home/nassaj/Project/nassaj-ops/scripts/runner/bin/nassaj-runner.sh >> /home/nassaj/Project/nassaj-ops/scripts/runner/logs/cron.log 2>&1
```
...
### الحالة الفعلية (2026-06-14)

| المكوّن | الحالة |
|--------|--------|
| محرّك bash (nassaj-ops) | ✅ مبنيّ ومُختبَر (121/121 اختبار يمران) |
| جسر TypeScript (nassaj-dev) | ✅ مبنيّ |
| overlay الواجهة | ✅ مبنيّ (RunnerControlBar، RunnerJourney) |
| **التفعيل على مشروع حيّ** | ⏳ مبنيّ ومُختبَر، التفعيل قيد القرار |

### ملفات المشاريع المسجَّلة

- `diwan.json`: **مسجَّل بـ enabled:false** — ينتظر موافقة المالك

### شروط التفعيل

1. **اعتماد ADR-RUNNER-BRIDGE-001** — الإطار النظري للجسر، قيد الاعتماد
2. **بناء overlay الواجهة** (`build:client`) + restart السيرفر
3. **قلب `enabled: true`** في `projects/diwan.json`
4. **إضافة سطر cron** يدوياً (لا يُثبَّت تلقائياً)

**الموقوف على:** قرار المالك النهائي. التفعيل مخطط تقريباً بعد منتصف 2026-06 لكن لا موعد قاطع.
...
| المصطلح | الشرح |
|--------|--------|
| **المنوال (runner)** | أوركستريتر التنفيذ الذاتي — يعمل كل ١٥ دقيقة |
...
| **artifact** | ملف النتيجة (build-signal.json، verdict-critique.json، إلخ) |
...
5. **أضِف سطر cron** — في طرفيتك (أنت بنفسك، لا يُثبَّت آلياً)
