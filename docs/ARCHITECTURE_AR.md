...
## من يرى محتوى الجلسة — رؤية على مستوى المشروع (ADR-052)

نسّاج أداة فريقية موثوقة: المخزن مشترك بيزياً والافتراض أن `public = مرئي للفريق`.

**قاعدة الرؤية:**

عضو الفريق يرى محتوى جلسة إذا تحقق **أي** من هذه:
1. **عضو مباشر** — مُشارك في الجلسة نفسها.
2. **أو يرى مشروع الجلسة:**
   - المشروع عامّ، **أو**
   - هو مُنشِئ المشروع، **أو**
   - هو عضو فيه، **أو**
   - شارك في **أي** جلسة من جلساته.

يُطبَّق في طبقة القراءة (`fetchHistory` والبحث) عبر دالة `isProjectPathVisibleToUser()`
التي تعطي نفس إجابة قائمة المشاريع `getVisibleProjectPaths()` — فلا تناقض بينهما.

**الأمان:** جلسة بمشروع خاص لمن لا ينتمي إليه = مخفية 403/404. أي متصل بلا هوية
(userId غير رقم صحيح) مرفوض **قبل أي فحص** — حتى مشروع عام مخفي عمن لم يسجّل دخوله.

(انظر `docs/decisions/052-session-visibility-project-scope.md` للتفاصيل الكاملة.)
...
`https://nassaj.alrukhaimi.com`) فإن انجرفت بيئة `ALLOWED_ORIGINS` لا تظهر صفحة بيضاء.
