...
- **✅ الثيمات في Appearance (C-MU-UX-THEMES/T-31) — منجزة ‏(27627a0):** محرّك إضافة `enhanced-appearance` ‏(Apache 2.0) منقول كميزة أصيلة في `src/lib/theme-presets.ts` + منتقي ثيم في إعدادات Appearance — ‏4 presets ‏(Claude/Cursor/Codex/Gemini) + محرر مخصص + «Default» افتراضي يعيد ثيم `index.css`؛ الوضع الداكن من ThemeContext، مفتاح `nassaj-theme-preset`، تطبيق مبكر بلا وميض، i18n ‏ar/en كاملة.
