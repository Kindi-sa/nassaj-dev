...
**🟡 شريحة B-MU-UX — طبقة العزو والحضور (UX layer):** مخطّطة (planned). تجعل مساحة العمل **المشتركة بالقصد** (إخوة، ثقة كاملة، وصول كامل) **مقروءة**: مَن يملك كل محادثة، مَن يعمل الآن، وفلترة المستخدم لعمله — إضافة **رؤية/عزو لا عزل**. بيانات الملكية موجودة في `session_participants` لكنها غير مستهلَكة من الواجهة. التفاصيل: `docs/workitems/PHASE-MU.md` (قسم B-MU-UX).
- **بوابة الشريحة:** أساس الـAPI **`B-MU-UX-API` يهبط أولاً** قبل بنود الواجهة (الشارة/الرسالة/الفلتر) المعتمدة عليه.
- **البنود:** `B-MU-UX-API` (كشف المالك/المشاركين) → `C-MU-UX-OWNER-BADGE` (شارة المالك بالشريط) ∥ `C-MU-UX-MSG-IDENTITY` (هويّة منسّق/وكيل بلون المالك) ∥ `B-MU-UX-PROJ-FILTER`+`C-MU-UX-PROJ-FILTER` (فلتر «مشاريعي/الكل»، الافتراضي «الكل»). مسار مستقلّ: `B-MU-UX-PRESENCE`+`C-MU-UX-PRESENCE` (حضور لحظي). مستقلّة: `B-MU-UX-GIT-ID` (هويّة git per-user — مؤلِّف + دفع)، `B-MU-UX-FIX-MIRROR` (تقييد معالِجات المرآة بالجلسة النشطة)، `B-MU-UX-FIX-WSROTATE` (إصلاح موت WS بعد تدوير التوكن).
- **ملاحظة التصميم:** مساحة مشتركة بالقصد (وصول كامل بين الإخوة)؛ هذه البنود **عزو/رؤية لا عزل**؛ الدفع per-user يتراجع **للحساب المشترك** عند غياب توكن المستخدم.
